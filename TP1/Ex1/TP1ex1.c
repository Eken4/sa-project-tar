/**
 * TP1 - Exercice 1 - Copie de fichier
 * Programme qui copie le contenu d'un fichier source dans un fichier destination
 */

#include<stdio.h>     // used for printf() function
#include<stdlib.h>    // used for exit() function 
#include<string.h>    // used for strerror() function
#include<errno.h>     // used for errno variable

#include<getopt.h>    // used for getopt_long() function
...
int main(int argc, char** argv)
{
  ...
  // Business logic must be implemented at this point
    ...
    // Ajout de la logique de copie de fichier
  int input_fd, output_fd;
  ssize_t bytes_read, bytes_written;
  char buffer[BUFFER_SIZE];

  // Ouverture du fichier source en lecture seule
  input_fd = open(bin_input_param, O_RDONLY);
  if (input_fd == -1) {
  ...
  output_fd = open(bin_output_param, O_WRONLY | O_CREAT | O_TRUNC, S_IRUSR | S_IWUSR | S_IRGRP | S_IROTH);
  if (output_fd == -1) {
      perror("Error opening or creating the destination file");
      close(input_fd);
      exit(EXIT_FAILURE);
  }

  // Lecture du contenu du fichier source et écriture dans le fichier destination
